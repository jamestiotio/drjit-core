...
// ====================================================================
//          API for extracting profile data from Dr.Jit programs
// ====================================================================

// A captured JIT profile (opaque)
typedef void *jit_profile_t;

// A single entry in a captured JIT profile (opaque)
typedef void *jit_event_t;

/// Begin profiling a region of code
extern JIT_EXPORT void jit_profile_begin();

/// Finish profiling, returns an opaque handle representing the captured data
extern JIT_EXPORT jit_profile_t jit_profile_end();

/// Release all memory associated with a captured profile
extern JIT_EXPORT jit_profile_free(jit_profile_t profile);

/// Compute the size of a captured profile (# of entries)
extern JIT_EXPORT size_t jit_profile_size(jit_profile_t profile);

/// Access a single event from a captured profile
extern JIT_EXPORT jit_event_t jit_profile_event(jit_profile_t profile, size_t entry);

/// Specify the type of event that was captured
extern JIT_EXPORT JIT_ENUM JitEventType jit_event_type(jit_event_t item);

/// Determine the Dr.Jit backend associated with the given event
extern JIT_EXPORT JitBackend jit_event_backend(jit_event_t item);

/// Compute the on-device runtime of a single event entry in milliseconds
extern JIT_EXPORT double jit_event_device_time(jit_event_t item);

/// Compute the Dr.Jit part of the kernel compile time in milliseconds
extern JIT_EXPORT double jit_event_kernel_frontend_time(jit_event_t item);

/// Compute backend-specific part of the kernel compile time in milliseconds
extern JIT_EXPORT double jit_event_kernel_backend_time(jit_event_t item);

/// Access the LLVM/PTX IR of a compiled kernel
extern JIT_EXPORT const char* jit_event_kernel_source(jit_event_t item);

/// Generate a mark in a captured profile
/// This function targets both the jit_profile_*() API and NVTX
...
/// This function targets both the jit_profile_*() API and NVTX
extern JIT_EXPORT void jit_profile_range_begin(const char *message);

/// Inform the profiler (if present) about the end of a tracked range
extern JIT_EXPORT void jit_profile_range_end();
